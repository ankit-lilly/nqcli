const highlightLib = typeof window !== "undefined" ? window.hljs ?? null : null;
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (processed.length <= MAX_HIGHLIGHT_LENGTH) {
      ...
      console.warn(`Skipping syntax highlighting (length=${processed.length}, hljs ready=${Boolean(highlightLib)})`);
    }
