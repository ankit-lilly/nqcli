import { type ConnectionConfig, queryEngineOptions } from "@/core";
...
export function createExportedConnection(
	...
	const dbUrl = connection.url.toLowerCase();
...
export async function parseExportedGraph(blob: Blob) {
	...
	const vertices = new Set(
		payload.vertices
			...
			.map(escapeIfPropertyGraphAndString)
			.map(createVertexId),
	);
...
function escapeIfPropertyGraphAndString(value: EntityRawId) {
	return typeof value !== "string" ? value : escapeDoubleQuotes(value);
}
