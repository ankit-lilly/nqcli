...
/**
 * The position accepts a value, but not one of this type, such as a numeric ID
 * where a language only supports string IDs.
 *
 * The value is reported rather than coerced: silently stringifying an ID or
 * coercing it would address a different entity than the caller asked for.
 */
 ...
 * because the same character can be legal elsewhere. For example, a control
 * character may be valid in a string value but invalid in an identifier.
 *
