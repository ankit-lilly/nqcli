...
describe("domain defaults", () => {
	it("uses USDM-aware defaults for known vertex families", () => {
		const study = resolveVertexStyle(createVertexType("Study"), undefined);
		const activity = resolveVertexStyle(
			createVertexType("Activity"),
			undefined,
		);
		const concept = resolveVertexStyle(
			createVertexType("BiomedicalConcept"),
			undefined,
		);
		const design = resolveVertexStyle(
			createVertexType("StudyDesign"),
			undefined,
		);

		expect(study.color).toBe("#e11d48");
		expect(study.borderWidth).toBe(2);
		expect(study.shape).toBe("round-hexagon");
		expect(design.color).toBe("#7c3aed");
		expect(activity.color).toBe("#0d9488");
		expect(concept.color).toBe("#16a34a");
		expect(concept.shape).toBe("diamond");
	});

	it("uses USDM-aware defaults for known edge families", () => {
		const version = resolveEdgeStyle(
			createEdgeType("has_latest_version"),
			undefined,
		);
		const schedule = resolveEdgeStyle(createEdgeType("nextEpoch"), undefined);

		expect(version.lineColor).toBe("#2563eb");
		expect(version.lineThickness).toBe(3);
		expect(schedule.lineColor).toBe("#d97706");
		expect(schedule.lineStyle).toBe("dashed");
	});

	it("lets user styles override domain defaults", () => {
		const style = resolveVertexStyle(createVertexType("Study"), {
			type: createVertexType("Study"),
			color: "red",
		});

		expect(style.color).toBe("red");
		expect(style.shape).toBe("round-hexagon");
	});
});
