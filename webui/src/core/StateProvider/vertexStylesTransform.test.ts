 ...
 *
 * Older local vertex style data can carry `"color": ""`. An empty color is not
 * a color: it overrides the app default through the plain spread in
 * `resolveVertexStyle`, and every consumer then renders nothing for it — a node
 * with no background, an icon that falls back to black. Blank colors are
 * therefore dropped at load time, letting the default apply.
 *
 * DO NOT delete these tests without confirming no stored style can carry a
 * blank color.
 */
...
describe("transformVertexStyles", () => {
	...
	it.each([
		...
		ShapeStyle,
		ShapeStyle,
	][])("coerces %s to %s in a stored map", (broken, expected) => {
