 ...
 *
 * Colors are persisted as optional strings, so older local state can carry
 * `"color": ""`. An empty value still overrides the app default through the
 * spread in `resolveVertexStyle`, leaving every consumer to render a node with
 * no background or an icon that falls back to black. Removing the field instead
