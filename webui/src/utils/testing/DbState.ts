...
export class DbState {
	...
	addTestableEdgeToGraph(edge: TestableEdge) {
		const graphEdge = edge.asEdge();
		this.edges.push(graphEdge);
		this.#activeSchema?.edges.push(mapEdgeToTypeConfig(graphEdge));
		for (const sourceVertexType of edge.source.types) {
			for (const targetVertexType of edge.target.types) {
				this.#activeSchema?.edgeConnections?.push({
					sourceVertexType,
