...
export const concentricLayout = {
	...
	transform: (_node: cytoscape.NodeSingular, position: unknown) => position, // transform a given node position. Useful for changing flow direction in discrete layouts
};
...
export const dagreLayoutTopToBottom = {
	...
	animationEasing: undefined, // easing of animation if enabled
	boundingBox: undefined, // constrain layout bounds; { x1, y1, x2, y2 } or { x1, y1, w, h }
	transform: (_node: cytoscape.NodeSingular, position: unknown) => position, // a function that applies a transform to the final node position
